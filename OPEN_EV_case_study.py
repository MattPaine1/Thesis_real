...
# tolerance band above the mean demand where limited charging is allowed
# expressed as a fraction of the mean demand (e.g. 0.1 = 10%)
demand_tolerance = 0.1
...
metrics = {
    'peak_network_demand': {}, # peak power imported at the grid interface
    'existing_demand_peak': {}, # peak demand from uncontrollable assets only
    'peak_imported_power': {}, # max increase in import above the base load
    'base_demand_at_additional_import': {}, # base demand when additional import occurs
...
def record_metrics(strategy, storage_assets, P_import, P_demand,
    ...
    # metrics
    P_import = np.asarray(P_import)
    P_demand_base = np.asarray(P_demand_base)
    diff_import_base = P_import - P_demand_base
    max_diff_idx = int(np.argmax(diff_import_base)) if diff_import_base.size else 0
    metrics['peak_network_demand'][strategy] = (
        float(np.max(P_import)) if P_import.size else 0.0
    )
    metrics['existing_demand_peak'][strategy] = (
        float(np.max(P_demand_base)) if P_demand_base.size else 0.0
    )
    if diff_import_base.size:
        metrics['peak_imported_power'][strategy] = float(diff_import_base[max_diff_idx])
        metrics['base_demand_at_additional_import'][strategy] = float(
            P_demand_base[max_diff_idx]
        )
    else:
        metrics['peak_imported_power'][strategy] = 0.0
        metrics['base_demand_at_additional_import'][strategy] = 0.0
...
def plot_performance_metrics(metrics, path): #print performance metrics for all strategies
    strats = list(metrics['peak_network_demand'].keys())
    if not strats:
        return
    ...
    def format_strategy_label(name):
        label_map = {
            ...
            'lp': 'LP',
            'composite': 'Comp',
            'composite_band': 'Comp-B',
            'composite_backlog': 'Comp-Bk',
            'composite_soft': 'Comp-S',
            'composite_mpc': 'Comp-MPC',
            'pareto': 'Par',
            'pareto_band': 'Par-B',
            'pareto_backlog': 'Par-Bk',
            'pareto_soft': 'Par-S',
            'pareto_mpc': 'Par-MPC',
        }
    ...
    for s in strats:
        print(f"Strategy '{s}':")
        peak_import = metrics['peak_network_demand'][s]
        base_peak = metrics['existing_demand_peak'][s]
        additional_import = metrics['peak_imported_power'][s]
        energy_var = metrics['energy_variability'][s]
        print(f"  Peak Network Demand: {peak_import} kW")
        print(f"  Existing Demand Peak: {base_peak} kW")
        print(f"  Peak Imported Power: {additional_import} kW")
    ...
    bar_plot(metrics['peak_network_demand'], 'Peak Network Demand (kW)',
             'peak_network_demand')
    bar_plot(metrics['existing_demand_peak'],
             'Existing Demand Peak (kW)',
             'existing_demand_peak')
    # Combined stacked bar: base demand and additional imported power
    additional_import = {s: metrics['peak_imported_power'][s] for s in strats}
    base_at_additional = {
        s: metrics['base_demand_at_additional_import'][s]
        for s in strats
    }
    base_vals = [base_at_additional[s] for s in strats]
    ...
    # O
    bar_plot(metrics['peak_imported_power'], 'Peak Imported Power (kW)',
             'peak_imported_power')
...
if run_opt ==1:
    ...
    for x in opt_type:
        ...
        if x == "composite_band":
            # Composite strategy allowing limited charging above mean demand
            P_ESs = np.zeros((T, N_ESs))
            E_state = E0_EVs.copy()
            t_arriv_dt = (tarriv_EVs * dt_ems / dt).astype(int)
            t_depart_dt = (tdepart_EVs * dt_ems / dt).astype(int)
            ...
            for t in range(T):
                ...
                if P_demand_base[t] <= base_mean:
                    ...
                    band_ratio = (base_mean + tolerance - P_demand_base[t]) / tolerance
                    P_valley = band_ratio * P_max_EV * len(connected)
                else:
                    P_valley = 0
                P_avail = min(P_network_cap, P_valley)
                if not connected or P_avail < min_chunk:
        ...
        if x == "pareto_band":
            # Pareto strategy with proportional band charging above mean demand
            P_ESs = np.zeros((T, N_ESs))
            E_state = E0_EVs.copy()
            t_arriv_dt = (tarriv_EVs * dt_ems / dt).astype(int)
            t_depart_dt = (tdepart_EVs * dt_ems / dt).astype(int)
        ...
        if x == "composite_band":
            pickled_data_COMP_BAND = (N_EVs, P_demand_base_pred_ems, P_compare, P_demand_base,\
                            Pnet_market, storage_assets, N_ESs, nondispatch_assets,\
                            time_ems, time, timeE, buses_Vpu)
            pickle.dump(pickled_data_COMP_BAND, open(join(path_string, normpath("EV_case_data_composite_band.p")), "wb"))

        if x == "composite_backlog":
            pickled_data_COMP_BACKLOG = (N_EVs, P_demand_base_pred_ems, P_compare, P_demand_base,\
                            Pnet_market, storage_assets, N_ESs, nondispatch_assets,\
                            time_ems, time, timeE, buses_Vpu)
            pickle.dump(pickled_data_COMP_BACKLOG, open(join(path_string, normpath("EV_case_data_composite_backlog.p")), "wb"))
        ...
        if x == "pareto_band":
            pickled_data_PAR_BAND = (N_EVs, P_demand_base_pred_ems, P_compare, P_demand_base,\
                            Pnet_market, storage_assets, N_ESs, nondispatch_assets,\
                            time_ems, time, timeE, buses_Vpu)
            pickle.dump(pickled_data_PAR_BAND, open(join(path_string, normpath("EV_case_data_pareto_band.p")), "wb"))

        if x == "pareto_backlog":
            pickled_data_PAR_BACKLOG = (N_EVs, P_demand_base_pred_ems, P_compare, P_demand_base,\
                            Pnet_market, storage_assets, N_ESs, nondispatch_assets,\
                            time_ems, time, timeE, buses_Vpu)
            pickle.dump(pickled_data_PAR_BACKLOG, open(join(path_string, normpath("EV_case_data_pareto_backlog.p")), "wb"))
...
# Load pickled data and plot
    ...
    for x in opt_type:
        ...
        if x == "composite_band":
            import_departata = pickle.load(open(join(path_string, normpath("EV_case_data_composite_band.p")), "rb"))

        if x == "composite_backlog":
            import_departata = pickle.load(open(join(path_string, normpath("EV_case_data_composite_backlog.p")), "rb"))

        if x == "pareto":
            import_departata = pickle.load(open(join(path_string, normpath("EV_case_data_pareto.p")), "rb"))

        if x == "pareto_band":
            import_departata = pickle.load(open(join(path_string, normpath("EV_case_data_pareto_band.p")), "rb"))

        if x == "pareto_backlog":
            import_departata = pickle.load(open(join(path_string, normpath("EV_case_data_pareto_backlog.p")), "rb"))
