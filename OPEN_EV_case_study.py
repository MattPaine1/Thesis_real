...
def ridge_regression_composite_backlog(
    ...
    yielding 66 unique combinations. For each performance metric a scatter plot
    of sampled weights versus the metric is produced together with the fitted
    ...
    for metric_name in metric_names:
        ...
        labels = ["Wait Weight", "Required Weight", "Price Weight"]
        for i, ax in enumerate(axes):
            ax.scatter(X[:, i], y, color="C0", s=10, label="Simulated")
            w_range = np.linspace(0, 1, 100)
            if i == 0:
                w_wait = w_range
                w_req = (1 - w_wait) / 2
                w_price = 1 - w_wait - w_req
            elif i == 1:
                w_req = w_range
                w_wait = (1 - w_req) / 2
                w_price = 1 - w_wait - w_req
                ...
                w_price = w_range
                w_wait = (1 - w_price) / 2
                w_req = 1 - w_wait - w_price
            X_pred = np.column_stack([w_wait, w_req, w_price])
            y_pred = model.predict(X_pred)
            ax.plot(w_range, y_pred, color="C1", label="Ridge fit")
            ax.set_xlabel(labels[i])
            ax.set_ylabel(metric_name)
        fig.tight_layout()
        fig.savefig(join(ridge_path, f"ridge_{metric_name}{save_suffix}"))
        plt.close(fig)
